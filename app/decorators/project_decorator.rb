class ProjectDecorator < Draper::Decorator
  ...
  def display_video_embed_url
    if source.video_url
      ...
      if url.include? 'vimeo'
        final = url.match(/[0-9]{8}/)
        "//player.vimeo.com/video/#{final[0]}" if final
      else
        final = url.match(/[A-Za-z0-9-_]{11}/)
        "//www.youtube.com/embed/#{final[0]}/?title=0&byline=0&portrait=0&autoplay=0&color=ffffff&badge=0&modestbranding=1&showinfo=0&border=0&controls=2" if final
      end
    end
  end
  ...
  def display_pledged
    number_to_currency source.pledged, precision: 0, unit: '€'
  end

  def display_goal
    number_to_currency source.goal, precision: 0, unit: '€'
  end
